...
type LibraryQuery struct {
	...
	withNamedVideos map[string]*VideoQuery
...
func (lq *LibraryQuery) QueryVideos() *VideoQuery {
	query := (&VideoClient{config: lq.config}).Query()
...
func (lq *LibraryQuery) FirstIDX(ctx context.Context) int {
...
func (lq *LibraryQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (lq *LibraryQuery) WithVideos(opts ...func(*VideoQuery)) *LibraryQuery {
	query := (&VideoClient{config: lq.config}).Query()
...
func (lq *LibraryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Library, error) {
	...
	if query := lq.withVideos; query != nil {
		if err := lq.loadVideos(ctx, query, nodes,
			func(n *Library) { n.Edges.Videos = []*Video{} },
			func(n *Library, e *Video) { n.Edges.Videos = append(n.Edges.Videos, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range lq.withNamedVideos {
		if err := lq.loadVideos(ctx, query, nodes,
			func(n *Library) { n.appendNamedVideos(name) },
			func(n *Library, e *Video) { n.appendNamedVideos(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (lq *LibraryQuery) loadVideos(ctx context.Context, query *VideoQuery, nodes []*Library, init func(*Library), assign func(*Library, *Video)) error {
	...
	query.Where(predicate.Video(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(library.VideosColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.library_videos
		if fk == nil {
			return fmt.Errorf(`foreign-key "library_videos" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "library_videos" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (lq *LibraryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(library.Table, library.Columns, sqlgraph.NewFieldSpec(library.FieldID, field.TypeInt))
	_spec.From = lq.sql
...
// WithNamedVideos tells the query-builder to eager-load the nodes that are connected to the "videos"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (lq *LibraryQuery) WithNamedVideos(name string, opts ...func(*VideoQuery)) *LibraryQuery {
	query := (&VideoClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if lq.withNamedVideos == nil {
		lq.withNamedVideos = make(map[string]*VideoQuery)
	}
	lq.withNamedVideos[name] = query
	return lq
}
